...
@Component({
  selector: 'app-tv-sets-collection',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './tv-sets-collection.component.html',
  styleUrl: './tv-sets-collection.component.css'
})



export class TvSetsCollectionComponent implements OnInit{
  ...
  applyFilters():void {
    ...
    this.filteredProducts = this.products.filter((product: { category: string }) => {
      return product.category === 'TVunits'; // Only include products with category "Beds"
    });
  }
