...
@Component({
  selector: 'app-coffe-tabel-collection',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './coffe-tabel-collection.component.html',
  styleUrl: './coffe-tabel-collection.component.css'
})
export class CoffeTabelCollectionComponent implements OnInit{
  ...
  applyFilters():void {
    ...
    this.filteredProducts = this.products.filter((product: { category: string }) => {
      return product.category === 'CoffeTabel'; // Only include products with category "Beds"
    });
  }
